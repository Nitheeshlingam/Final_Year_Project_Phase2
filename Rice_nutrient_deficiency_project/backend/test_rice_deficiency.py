...
class RiceDeficiencyTester:
    def __init__(self):
        ...
        # Load scaler if available (used by improved classical ML training)
        self.scaler = None
        ...
        if os.path.exists(scaler_path):
            try:
                ...
                print(f"⚠️ Failed to load feature scaler from {scaler_path}: {e}")
        ...
        try:
            ...
            if self.scaler is not None:
                try:
                    ...
                    print(f"⚠️ Feature scaling failed, proceeding without scaling: {e}")
    ...
    def test_deep_learning(self, image_array):
        ...
        try:
            # Prefer improved model if available
            ...
            # Resize input dynamically to model's expected size
            try:
                expected_h, expected_w = self._dl_model.input_shape[1], self._dl_model.input_shape[2]
            except Exception:
                expected_h, expected_w = 224, 224
            if image_array.shape[0] != expected_h or image_array.shape[1] != expected_w:
                img_resized = cv2.resize(image_array, (expected_w, expected_h), interpolation=cv2.INTER_AREA)
            else:
